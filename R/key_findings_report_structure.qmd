...
coefs <- extract_glm_coefs(mod_2)

black_coef <- extract_glm_coefs(mod_2, only_sig = T) %>%
  filter(variable == "Ethnicity_collapsedBlack/African/Caribbean/Black British:outsourcing_statusOutsourced") %>%
...
However there is nuance in the groups. There is evidence to suggest that people who are Black and outsourced are less likely to be in the high income group (OR = `r black_coef`x). 
