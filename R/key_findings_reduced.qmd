...
```{r}
# invesitgate Has_Degree. This can go in cleaning script
...
# make "no" the reference category for Has_Degree
data <- data %>%
  mutate(
    Has_Degree = forcats::fct_relevel(Has_Degree, "No")
  )
...
# 
# ttest <- sjstats::weighted_ttest(Age ~ outsourcing_status + NatRepemployees, data) # for some reason this doesn't exist anymore?
...
The higher concentration of younger workers identified above appears to be driven primarily by the 'outsourced' and 'high indicator' groups, whilst the 'likely agency' group follows a similar pattern to the non-outsourced group.[^5]

[^5]: [outputs/data/age_stats_2.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/age_stats_2.csv)
...
[^8]: [outputs/data/ethnicity_stats_1.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/ethnicity_stats_1.csv) & [outputs/data/ethnicity_binary_o-status_inferential_tab.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/ethnicity_binary_o-status_inferential_tab.csv)
...
Comparison of ethnicities indicates that some groups are statistically more likely to be outsourced than others[^9]:

[^9]: [outputs/data/ethnicity_model_inferential.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/ethnicity_model_inferential.csv)
...
Breaking down by outsourcing group helps to separate out the *type* of outsourced work people from the ethnicities identified above engage in.[^10] Compared to White British workers,

[^10]: [outputs/data/ethnicity_ogroup_inferential_tab.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/ethnicity_ogroup_inferential_tab.csv)
...
Exploring *types* of outsourced work indicates that the pattern observed above applies evenly to the different outsourcing groups.[^13] Compared to people born in the UK, people not born in the UK are:

[^13]: [outputs/data/bornuk_ogroup_inferential_tab.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/bornuk_ogroup_inferential_tab.csv) & [/outputs/data/arrival_in_UK_stats_2.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/arrival_in_UK_stats_2.csv)
...
Exploring the intersection of ethnicity and arrival time reveals some patterns whereby the likelihood of a person being outsourced is related to the combinations of ethnicity and whether they were born in the UK.[^14] The plot below shows that

[^14]: [outputs/data/bornUK_binary_contrasts.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/bornUK_binary_contrasts.csv)
...
Similarly, the plot below shows that[^15]

[^15]: [outputs/data/region_stats_2.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/region_stats_2.csv)
...
Exploring these categories[^16] confirms that

[^16]: [outputs/data/region_stats_3.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/region_stats_3.csv)
...
[^17]: [outputs/data/bornUK_collapsed_contrasts_2.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/bornUK_collapsed_contrasts_2.csv)
...
The plot below shows the proportion of workers within each region who are outsourced.[^18]

[^18]: [outputs/data/region_stats_2.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/region_stats_2.csv)
...
The table and plot below show descriptive statistics on income and its distribution for outsourced and non-outsourced people. Regression analysis shows that **outsourced workers are on average paid £`r abs(round(coef_table['outsourcing_statusOutsourced','Estimate'],0))` less than non-outsourced workers**.[^20]

[^20]: [outputs/data/income_stats_o-status.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/income_stats_o-status.csv) & [outputs/data/model_income_by_o-status.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/model_income_by_o-status.csv)
...
```{r output=FALSE}
mod_2 <- lm(income_annual_all ~ Age + Gender + Ethnicity_collapsed + Region + Has_Degree + outsourcing_status + SectorName_short, income_data, weights = NatRepemployees)
# summary(mod_2)
...
rownames(coef_table) <- coef_table$variable
sig_coefs <- extract_lm_coefs(mod_3, only_sig = T)

write_csv(coef_table, file="../outputs/data/model_2_income_by_o-status.csv")
```

This difference increases to **£`r abs(round(coef_table['outsourcing_statusOutsourced','Estimate'],0))`** when we take into account Age, Gender, Ethnicity, Region, and Arrival Time. [^21] This analysis shows that all other variables, apart from Age, are in some way relevant to income. On average,

[^21]: [outputs/data/model_2_income_by_o-status.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/model_2_income_by_o-status.csv)

-   Men earn £`r abs(round(coef_table['GenderMale','Estimate'],0))` more than women.
-   South Asian workers earn £`r abs(round(coef_table['Ethnicity_collapsedEast Asian','Estimate'],0))` less than White British workers
...
-   Workers in all non-London regions earn less than workers in London
    ...
    -   Yorkshire and the Humber: -£`r abs(round(coef_table['RegionYorkshire and the Humber','Estimate'],0))` than people born in the UK.
...
-   People who arrived within the last 30 years earn £`r abs(round(coef_table['BORNUK_labelledWithin the last 30 years','Estimate'],0))` more
...
Next we explore differences by outsourcing group. The table and plot below show descriptive statistics on income and its distribution for outsourced groups. Regression analysis shows that **outsourced workers are on average paid £`r abs(round(coef_table['outsourcing_groupOutsourced','Estimate'],0))` less than non-outsourced workers**, while no differences are evident for the likely agency and high indicators groups.[^22]

[^22]: [outputs/data/income_stats_o-group.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/income_stats_o-group.csv) & [outputs/data/model_income_by_o-group.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/model_income_by_o-group.csv)
...
```{r output=FALSE}
mod_2 <- lm(income_annual_all ~ Age + Gender + Ethnicity_collapsed + Has_Degree + Region + outsourcing_group + SectorName_short, income_data, weights = NatRepemployees)
# summary(mod_2)
...
However, when controlling, as before, for Age, Gender, Ethnicity, Arrival Time, Region, and Sector[^23] we find

[^23]: [outputs/data/model_2_income_by_o-group.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/model_2_income_by_o-group.csv)
...
Other effects in this model are largely the same as in the status model, however note that the 'Within the last 3 years effect' is not present in this model. Given this effect was only just significant previously, the fact that it disappears when we separate outsourced from likely agency suggests that the variance this level explained is now explained by the distinction between outsourced and likely agency. That is, in the previous model, it was likely that 'last 3 years' level of the arrival time factor was capturing variance explained by being an agency worker specifically. When we consider agency workers explicitly, the within 3 year time frame is less relevant in explaining income (i,e, it is the fact of being an agency worker that results in less pay, not when they arrived). The fact that the 'within the last year' timeframe is significant suggests that people arriving recently suffer a pay penalty, even after accounting for what outsourcing group they fall into.

### Gender pay gap

Use a mixed model to explore how the effect of gender varies by sector. Note due to small sample sizes we can only consider Male and Female genders.

```{r output=FALSE}
# explore how the effect of gender varies by sector using a mixed model
test_data <- income_data %>%
  filter(!is.na(SectorName_short))  %>%
  filter(Gender %in% c("Male","Female"))

# base model - no random effects
base <- lm(income_annual_all ~ Age + Gender + Ethnicity_collapsed + BORNUK_labelled + Has_Degree + Region + outsourcing_status, test_data, weights = NatRepemployees)
summary(base)

# random intercept
base_2 <- lmerTest::lmer(income_annual_all ~ Age + Gender + Ethnicity_collapsed + BORNUK_labelled + Has_Degree + Region + outsourcing_status + (1|SectorName_short), test_data, weights = NatRepemployees)

# random intercept + slope
mod <- lmerTest::lmer(income_annual_all ~ Age + Gender + Ethnicity_collapsed + BORNUK_labelled + Has_Degree + Region + outsourcing_status + (Gender|SectorName_short), test_data, weights = NatRepemployees)
summary(mod)

# check whether random effects improve fit it - they do, best is with slope
anova(mod,base_2, base)
...
random_effects <- ranef(mod)$SectorName_short

total_effects <- fixed_effects["GenderMale"] + random_effects["GenderMale"]
```

A mixed model allowing the effect of Gender to vary by sector is a better fit to the data than a model with just a random intercept and a regular linear model. This suggests that the differences between sectors in the effect of Gender is important.

The fixed effect shows that on average, across sectors, men earn £`r round(fixed_effects["GenderMale"],0)` more than women.

The plot below shows the random effects. This is the relative influence of gender in sectors compared to the average effect of gender across sectors. It shows the sectors where the effect on pay of being male is greater than the average across sectors (red) and the sectors where the effect of being male is less than average (blue). It shows that some of the most unfair sectors are 'Other service activities', 'Wholesale and retail trade', and 'pressional, scientific and technical activities', amongst others, and some of the least unfair sectors are Accommodation and food service activities, transportation and storage, and human health and social work activities (note however, as shown in the second chart below, that in all **all** sectors being male is associated with being paid more). 


```{r}
# get confidence intervals to effects
test <- broom.mixed::tidy(mod, effects="ran_vals",conf.int=TRUE)

# plot them
test %>%
  filter(term!="(Intercept)") %>%
  mutate(
    positive = factor(ifelse(estimate>0, 0,1)),
    level = forcats::fct_reorder(level, estimate)
  ) %>%
  ggplot(aes(level,estimate, colour=positive)) +
  geom_point() +
  ...
  labs(caption = "Note: Values indicate the influence of outsourcing within the sector relative to the average influence across sectors") +
  xlab("Sector")

```

Consideration of the total effect indicates that when controlling for other factors, being Male is assocaited with higher pay in all sectors.

```{r}
total_effects %>%
  mutate(
    Sector = forcats::fct_reorder(rownames(total_effects), GenderMale, .desc=F)
  ) %>%
  ggplot(aes(Sector, GenderMale)) +
  geom_point() +
  coord_flip() +
  theme_minimal() +
  ylab("Total effect") +
  labs(caption = "Values represent the total effect (i.e., fixed + random effect)")
```
...
```{r output=FALSE}
mod <- glm(income_group ~ Age + Gender + Ethnicity_collapsed + BORNUK_labelled + Region + Has_Degree + outsourcing_status, data, weights = NatRepemployees, family = "quasibinomial")
summary(mod)
```
...
And less likely to be in the low income group if they are:

-   Male
-   Arrived within the last 20 or 30 years
-   Have a degree.
...
<!-- mod <- glm(income_group ~ Age + Gender * Ethnicity_collapsed + BORNUK_labelled + Has_Degree +  Region, subset, weights = NatRepemployees, family = "quasibinomial") -->

<!-- summary(mod) -->

<!-- mod <- lm(income_annual_all ~ Age + Gender * Ethnicity_collapsed*outsourcing_status + Has_Degree + BORNUK_labelled + Region, income_data, weights = NatRepemployees) -->
...
<!-- sjPlot::plot_model(mod, type="pred", terms = c("Ethnicity_collapsed","Gender"), dodge=0.5) + -->

<!--   coord_flip() -->

<!-- sjPlot::plot_model(mod, type="int", terms = c("Gender","Ethnicity_collapsed"), dodge=0.5) + -->
...
<!-- mod <- glm(income_group ~ Age + Gender * Ethnicity_collapsed + BORNUK_labelled + Region, subset, weights = NatRepemployees, family = "quasibinomial") -->
...
data <- data %>%
  mutate(
    SectorName_labelled = case_when(SectorName_labelled == "NA" ~ NA,
                                    SectorName_labelled == "Na" ~ NA,
                                    TRUE ~ SectorName_labelled),
    SectorName_short = case_when(SectorName_short == "NA" ~ NA,
                                 SectorName_short == "Na" ~ NA,
                                    TRUE ~ SectorName_short),
    Majorgroupcode_labelled = case_when(Majorgroupcode_labelled == "NA" ~ NA,
                                        Majorgroupcode_labelled == "Na" ~ NA,
                                TRUE ~ Majorgroupcode_labelled)
  )
```



Here we explore what proportion of workers in each sector are outsourced.[^24]

[^24]: [outputs/data/sector_summary_3.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/sector_summary_3.csv)
...
Exploring this by type of outsourced worker shows that for all sectors, the majority of outsourced workers fall into the 'outsourced' group.[^25]

[^25]: [outputs/data/sector_summary_o-group.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/sector_summary_o-group.csv)
...
#### Variations in pay - outsourcing status

The plot below shows the average income by sector.[^26]

[^26]: [outputs/data/sector_summary_pay_overall.csv](https://github.com/Project-X-UK/jrf_nat_rep/tree/main/outputs/data/sector_summary_pay_overall.csv)

```{r}
sector_pay_summary <- data %>%
  filter(income_drop_all == 0 & !is.na(income_annual_all)) %>%
  ...
  mutate(
    SectorName_short = forcats::fct_reorder(SectorName_short, wtd_avg_income, .desc=FALSE)
  )

ypos <- max(sector_pay_summary$wtd_avg_income) * 1.2
ggplot(sector_pay_summary, aes(SectorName_short, wtd_avg_income)) +
  geom_col() +
  geom_text(aes(x=SectorName_short, y=ypos, label = paste0("N = ", N)), hjust=1, nudge_y = 15) +
  coord_flip() +
  theme_minimal() +
  xlab("Sector") + ylab("Weighted average income")

write_csv(sector_pay_summary, file="../outputs/data/sector_summary_pay_overall.csv")

```

```{r sector-multimod, output=FALSE}
test_data <- income_data %>%
  filter(!is.na(SectorName_short))
...
# random intercept
base_2 <- lmerTest::lmer(income_annual_all ~ Age + Gender + Ethnicity_collapsed + BORNUK_labelled + Region + outsourcing_status + (1|SectorName_short), test_data, weights = NatRepemployees)

# random intercept + slope
mod <- lmerTest::lmer(income_annual_all ~ Age + Gender + Ethnicity_collapsed + BORNUK_labelled + Region + outsourcing_status + (outsourcing_status|SectorName_short), test_data, weights = NatRepemployees)
summary(mod)
...
A multilevel model predicting income from outsourcing status (accounting for other variables), with random intercepts and random slopes for outsourcing status by sector indicates overall a significant negative effect of outsourcing status; by this estimation people who are outsourced earn approximately £2774 less than people who are not outsourced. The random effects indicate that influence of being outsourced varies with sector. For example, for people working in 'Wholesale and retail trade', the pay for outsourced workers os being outsourced and 'accommodation and food service activities', the effect of being outsourced on pay is **higher** than the average effect across sectors. Note that both of these sectors fall into the bottom three sectors overall for pay. For people working in 'Information and communication' and 'Education', the effect of being outsourced on pay is **lower** than the average across sectors. This shows that penalty to pay associated with outsourcing is less severe in some sectors and more severe in others.

::: callout-note
The plots below cannot be interpreted inferentially to determine "significance". They simply represent the variation in the estimate of outsourcing by sector. The thing to take away from them is that impact on pay of being outsourced is less severe in some sectors and more severe in others, but we cannot say from them that e.g., "sector A has significantly worse pay than sector B."
:::
...
total_effects <- fixed_effects["outsourcing_statusOutsourced"] + random_effects["outsourcing_statusOutsourced"]
```
...
test %>%
  filter(term!="(Intercept)") %>%
  mutate(
    positive = factor(ifelse(estimate>0, 1,0)),
    level = forcats::fct_reorder(level, estimate)
  ) %>%
  ggplot(aes(level,estimate, colour=positive)) +
  geom_point() +
...
Consideration of the total effect indicates that when controlling for other factors, outsourced workers in all sectors apart from 'Wholesale and retail trade' have lower pay than non-outsourced workers.

```{r}
total_effects %>%
  mutate(
    Sector = forcats::fct_reorder(rownames(total_effects), outsourcing_statusOutsourced, .desc=F)
  ) %>%
  ggplot(aes(Sector, outsourcing_statusOutsourced)) +
  geom_point() +
  coord_flip() +
  theme_minimal() +
  ylab("Total effect") +
...
[^27]: [outputs/data/sector_summary_pay.csv](https://github.com/Project-X-UK/jrf_nat_rep/tree/main/outputs/data/sector_summary_pay.csv)
...
##### Pay as function of non-outsourced^[[outputs/data/sector_summary_pay_diff.csv)](https://github.com/Project-X-UK/jrf_nat_rep/tree/main/outputs/data/sector_summary_pay_diff.csv)]

Below we plot, for each sector, outsourced workers' pay as proportion of non-outsourced workers' pay. The plot shows that on average, outsourced workers are paid more in Agriculture..., Arts..., and Wholesale trade... They're paid the same in Financial... and Professional..., and less in all other sectors. **Note that these averages do not take into account any other variables that are relevant to income.**


```{r}
# here we explore pay as function of the non-outsourced pay
sector_summary_pay <- data %>%
  filter(income_drop_all == 0 & !is.na(income_annual_all)) %>%
  group_by(SectorName, SectorName_labelled, outsourcing_status) %>%
  summarise(
    ...
    avg_income = mean(income_annual_all, na.rm=T),
    wtd_avg_income = weighted.mean(income_annual_all, w = NatRepemployees, na.rm=T),
    # sd = weighted_sd(income_annual_all, w = NatRepemployees, na.rm=T)
  ) %>% 
  ungroup() %>%
  group_by(SectorName) %>%
  mutate(
...
wider <- sector_summary_pay %>%
  select(SectorName_short, n, outsourcing_status, wtd_avg_income) %>%
  pivot_wider(names_from = outsourcing_status, values_from = c(wtd_avg_income, n)) %>%
  mutate(
    # Outsourced = ifelse(is.na(Outsourced), 0, Outsourced),
    Pay_diff = 100 * (wtd_avg_income_Outsourced/`wtd_avg_income_Not outsourced`),
    alt_pay_diff = ((`wtd_avg_income_Not outsourced` -wtd_avg_income_Outsourced)/`wtd_avg_income_Not outsourced`)
  )

write_csv(wider, file="../outputs/data/sector_summary_pay_diff.csv")
...
plot_data <- wider %>%
  ...
  ungroup() %>%
  mutate(
    SectorName_short = forcats::fct_reorder(SectorName_short, Pay_diff, .desc = FALSE)
  )

annotation_df <- plot_data %>%
  #filter(outsourcing_status == "Not outsourced") %>%
  select(SectorName_short, n_Outsourced) %>%
  group_by(SectorName_short) %>%
    mutate(
    ypos = max(plot_data$Pay_diff, na.rm=T) * 1.2
  ) 
...
plot_data %>%
  ...
  #   SectorName = as.factor(SectorName)
  # ) %>%
  ggplot(., aes(Pay_diff,SectorName_short)) + #, size = N, colour = outsourcing_status)) +
  geom_point(position = "dodge") + 
  ...
  # scale_x_continuous(breaks=seq(0,max(plot_data$wtd_avg_income, na.rm=T), 10000)) +
  # scale_colour_manual(values=colours) +
  geom_text(inherit.aes=F,data=annotation_df, aes(x=ypos, y=SectorName_short, label = paste0("n = ", n_Outsourced)), hjust=1) +
  guides(size=FALSE) + # remove size legend as gauging size is difficult 
  xlab("Outsourced pay as percentage of non-outsourced pay") + ylab("Sector") +
  labs(caption = "Note: n in indicates the size of the outsourced workforce in the sector")

# alternative way of presenting it
# could also think about expressing it, e.g. in terms of percentage less...
...
#   #   SectorName = as.factor(SectorName)
#   # ) %>%
#   ggplot(., aes(alt_pay_diff,SectorName_short)) + #, size = N, colour = outsourcing_status)) +
#   geom_point(position = "dodge") + 
#   # geom_errorbar(aes(xmin = min, xmax = max)) +
#   theme_minimal() +
#   theme(legend.position = "bottom",
...
##### Pay as function of non-outsourced

Exploring this as a proportion of the non-outsourced pay.^[[outputs/data/sector_summary_pay_diff_ogroup.csv](https://github.com/Project-X-UK/jrf_nat_rep/tree/main/outputs/data/sector_summary_pay_diff_ogroup.csv)]

Below we plot, for each sector, outsourced workers' pay as proportion of non-outsourced workers' pay. **Note that these averages do not take into account any other variables that are relevant to income.**
...
sector_summary_pay <- data %>%
  ...
  # Compute the 'not outsourced' reference value
  group_by(SectorName) %>%
  mutate(
    not_outsourced_wtd_avg_income = wtd_avg_income[outsourcing_group == "Not outsourced"]
  ) %>%
  # Calculate the proportion relative to the 'not outsourced' value
  mutate(
    wtd_avg_income_proportion = ifelse(outsourcing_group == "Not outsourced", 100, 100 * wtd_avg_income / not_outsourced_wtd_avg_income)
  ) %>%
  select(-not_outsourced_wtd_avg_income) #

write_csv(sector_summary_pay, file="../outputs/data/sector_summary_pay_diff_ogroup.csv")


levels <- sector_summary_pay %>%
  drop_na(SectorName_short) %>%
  droplevels() %>%
  filter(outsourcing_group == 'Outsourced') %>%
  mutate(SectorName_short = forcats::fct_reorder(SectorName_short, wtd_avg_income_proportion, .desc = FALSE))

plot_data <- sector_summary_pay %>%
  mutate(
    SectorName_short = factor(SectorName_short, levels = levels(levels$SectorName_short)),
        )
...
annotation_df <- plot_data %>%
  ...
  group_by(SectorName_short) %>%
    mutate(
      N = sum(n),
    ypos = max(plot_data$wtd_avg_income_proportion, na.rm=T) * 1.2
  ) 
...
plot_data %>%
  ...
  #   SectorName = as.factor(SectorName)
  # ) %>%
  ggplot(., aes(wtd_avg_income_proportion, SectorName_short, color = outsourcing_group)) + #, size = N, colour = outsourcing_status)) +
  geom_point(position = "dodge") + 
  # geom_errorbar(aes(xmin = min, xmax = max)) +
  theme_minimal() +
  theme(legend.position = "bottom",
  ...
  geom_text(inherit.aes=F,data=annotation_df, aes(x=ypos, y=SectorName_short, label = paste0("N = ", N)), hjust=1) +
  guides(size=FALSE) + # remove size legend as gauging size is difficult 
  xlab("Percentage of non-outsourced pay") + ylab("Sector") +
  labs(caption = "Note: n in indicates the size of the outsourced workforce in the sector") +
  scale_colour_manual(values=better_colours)
```
...
```{r output=FALSE}
# os status
mod <- lm(income_annual_all ~ Age + Gender + Ethnicity_collapsed + BORNUK_labelled + Has_Degree + Region + outsourcing_status*income_group, income_data, weights = NatRepemployees)
# summary(mod)

sjPlot::plot_model(mod, type = "int")

# os group
mod <- lm(income_annual_all ~ Age + Gender + Ethnicity_collapsed + BORNUK_labelled + Has_Degree +  Region + outsourcing_group*income_group, income_data, weights = NatRepemployees)
# summary(mod)

sjPlot::plot_model(mod, type = "int")
```

Although a model predicting income from outsourcing status and income group (controlling for other variables) indicates that pay does not depend on an interaction between a person's outsourcing status and income group, the plots below are informative in displaying variation by sector.[^29]

[^29]: [outputs/data/sector_summary_paysplit_o-status.csv](https://github.com/Project-X-UK/jrf_nat_rep/tree/main/outputs/data/sector_summary_paysplit_o-status.csv) & [outputs/data/sector_summary_paysplit_o-group_pay.csv](https://github.com/Project-X-UK/jrf_nat_rep/tree/main/outputs/data/sector_summary_paysplit_o-group_pay.csv)
...
The plot below shows the proportion of each major occupation group that is outsourced.[^30]

[^30]: [outputs/data/majorgroupcode_summary_o-status.csv](https://github.com/Project-X-UK/jrf_nat_rep/tree/main/outputs/data/majorgroupcode_summary_o-status.csv)
...
Below these proportions are further broken down by outsourcing group.[^31]

[^31]: [outputs/data/majorgroupcode_summary_o-group.csv](https://github.com/Project-X-UK/jrf_nat_rep/tree/main/outputs/data/majorgroupcode_summary_o-group.csv)
...
The plot below shows the average income by major occupation group.[^32]

[^32]: [outputs/data/majorgroupcode_summary_pay_overall.csv](https://github.com/Project-X-UK/jrf_nat_rep/tree/main/outputs/data/majorgroupcode_summary_pay_overall.csv)

```{r}
mgc_pay_summary <- data %>%
  filter(income_drop_all == 0 & !is.na(income_annual_all)) %>%
  ...
  mutate(
    Majorgroupcode_labelled = forcats::fct_reorder(Majorgroupcode_labelled, wtd_avg_income, .desc=FALSE)
  )

ypos <- max(mgc_pay_summary$wtd_avg_income) * 1.2

ggplot(mgc_pay_summary, aes(Majorgroupcode_labelled, wtd_avg_income)) +
  geom_col() +
  geom_text(aes(x=Majorgroupcode_labelled, y=ypos, label = paste0("N = ", N)), hjust=1, nudge_y = 15) +
  coord_flip() +
  theme_minimal() +
  xlab("Sector") + ylab("Weighted average income")

write_csv(mgc_pay_summary, file="../outputs/data/majorgroupcode_summary_pay_overall.csv")

```

```{r majorgroup-multimod, output=FALSE}
test_data <- income_data %>%
  filter(!is.na(SectorName_short))

# base model - no random effects
base <- lm(income_annual_all ~ Age + Gender + Ethnicity_collapsed +  Has_Degree + BORNUK_labelled + Region + outsourcing_status, test_data, weights = NatRepemployees)
summary(base)

# random intercept
base_2 <- lmerTest::lmer(income_annual_all ~ Age + Gender + Ethnicity_collapsed + Has_Degree + BORNUK_labelled + Region + outsourcing_status + (1|Majorgroupcode_labelled), test_data, weights = NatRepemployees)
summary(base_2)

# random intercept + slope
mod <- lmerTest::lmer(income_annual_all ~ Age + Gender + Ethnicity_collapsed + Has_Degree + BORNUK_labelled + Region + outsourcing_status + (outsourcing_status|Majorgroupcode_labelled), test_data, weights = NatRepemployees)
summary(mod)
...
A multilevel model predicting income from outsourcing status (accounting for other variables), with random intercepts and random slopes for outsourcing status by major occupation indicates overall a significant negative effect of outsourcing status; by this estimation people who are outsourced earn approximately £`r abs(round(coefs["outsourcing_statusOutsourced", "Estimate"],2))` less than people who are not outsourced. The random effects indicate that influence of being outsourced varies with major group. For example, for people who work in 'Sales and customer service', 'elementary occupations' and 'caring, leisure and other service occupations' the effect of being outsourced on pay is **higher** than the average effect across sectors. For people who work in 'professional occupations', 'associate professional occupations', and 'managers, directors and senior officials' the effect of being outsourced on pay is **lower** than the average across sectors.

As for the sector breakdown, this shows that the penalty to pay associated with outsourcing is less severe in some sectors and more severe in others.

```{r}
fixed_effects <- fixef(mod)
random_effects <- ranef(mod)$Majorgroupcode_labelled
...
test %>%
  ...
  ggtitle("Random effects") +
  labs(caption = "Note: Values indicate the influence of outsourcing within the sector relative to the average influence across sectors") +
  xlab("Major group code")

```

Consideration of the total effect indicates that when controlling for other factors, outsourced workers in all major groups apart from 'Sales and customer service occupations' have lower pay than non-outsourced workers.

```{r}
total_effects %>%
  mutate(
    `Major group` = forcats::fct_reorder(rownames(total_effects), outsourcing_statusOutsourced, .desc=F)
  ) %>%
  ggplot(aes(`Major group`, outsourcing_statusOutsourced)) +
  geom_point() +
  coord_flip() +
  theme_minimal() +
  ylab("Total effect") +
...
```{r include=FALSE}
sjPlot::plot_model(mod, type = "re", show.values = F)
```

Looking at absolute differences in pay by major occupation indicates that outsourced workers in most groups are paid less than non-outsourced workers.[^33]

[^33]: [outputs/data/mgc_summary_pay.csv](https://github.com/Project-X-UK/jrf_nat_rep/tree/main/outputs/data/mgc_summary_pay.csv)
...
##### Pay as function of non-outsourced^[[outputs/data/mgc_summary_pay_diff.csv)](https://github.com/Project-X-UK/jrf_nat_rep/tree/main/outputs/data/mgc_summary_pay_diff.csv)]

Below we plot, for each major group code, outsourced workers' pay as proportion of non-outsourced workers' pay. The plot shows that on average, outsourced workers are paid more in Sales..., Skilled trades..., and Administrative... and less in all other major group codes. **Note that these averages do not take into account any other variables that are relevant to income.**


```{r}
# here we explore pay as function of the non-outsourced pay
wider <- mgc_summary_pay %>%
  select(Majorgroupcode_labelled, n, outsourcing_status, wtd_avg_income) %>%
  pivot_wider(names_from = outsourcing_status, values_from = c(wtd_avg_income, n)) %>%
  mutate(
    # Outsourced = ifelse(is.na(Outsourced), 0, Outsourced),
    Pay_diff = 100 * (wtd_avg_income_Outsourced/`wtd_avg_income_Not outsourced`)
...
plot_data <- wider %>%
  ...
  ungroup() %>%
  mutate(
    Majorgroupcode_labelled = forcats::fct_reorder(Majorgroupcode_labelled, Pay_diff, .desc = FALSE)
  )

annotation_df <- plot_data %>%
  #filter(outsourcing_status == "Not outsourced") %>%
  select(Majorgroupcode_labelled, n_Outsourced) %>%
  group_by(Majorgroupcode_labelled) %>%
    mutate(
    ypos = max(plot_data$Pay_diff, na.rm=T) * 1.2
  ) 
...
plot_data %>%
  ...
  #   SectorName = as.factor(SectorName)
  # ) %>%
  ggplot(., aes(Pay_diff,Majorgroupcode_labelled)) + #, size = N, colour = outsourcing_status)) +
  geom_point(position = "dodge") + 
  # geom_errorbar(aes(xmin = min, xmax = max)) +
  theme_minimal() +
  theme(legend.position = "bottom",
  ...
  # scale_x_continuous(breaks=seq(0,max(plot_data$wtd_avg_income, na.rm=T), 10000)) +
  # scale_colour_manual(values=colours) +
  geom_text(inherit.aes=F,data=annotation_df, aes(x=ypos, y=Majorgroupcode_labelled, label = paste0("n = ", n_Outsourced)), hjust=1) +
  guides(size=FALSE) + # remove size legend as gauging size is difficult 
  xlab("Outsourced pay as percentage of non-outsourced pay") + ylab("Major occupation") +
  labs(caption = "Note: n in indicates the size of the outsourced workforce in the major occ group")

write_csv(wider, file="../outputs/data/mgc_summary_pay_diff.csv")
```

#### Variations in pay - outsourcing group

Breaking this down by group indicates that there is variation between types of outsourcing in terms of pay. The 'outsourced' group is often (though not always) the least well-paid out of the outsourcing groups.[^34]

[^34]: [outputs/data/mgc_summary_pay_group.csv](https://github.com/Project-X-UK/jrf_nat_rep/tree/main/outputs/data/mgc_summary_pay_group.csv)
...
##### Pay as function of non-outsourced pay^[[outputs/data/mgc_summary_pay_diff_o-group.csv)](https://github.com/Project-X-UK/jrf_nat_rep/tree/main/outputs/data/mgc_summary_pay_diff_o-group.csv)]

Below we plot, for each major group code, outsourced workers' pay as proportion of non-outsourced workers' pay. **Note that these averages do not take into account any other variables that are relevant to income.** 
...
levels <- sector_summary_pay %>%
  ...
  droplevels() %>%
  filter(outsourcing_group == 'Outsourced') %>%
  mutate(SectorName_short = forcats::fct_reorder(Majorgroupcode_labelled, wtd_avg_income_proportion, .desc = FALSE))

plot_data <- sector_summary_pay %>%
  mutate(
    Majorgroupcode_labelled = factor(Majorgroupcode_labelled, levels = levels(levels$Majorgroupcode_labelled)),
        )
...
plot_data %>%
  ...
  #   SectorName = as.factor(SectorName)
  # ) %>%
  ggplot(., aes(wtd_avg_income_proportion, Majorgroupcode_labelled, color = outsourcing_group)) + #, size = N, colour = outsourcing_status)) +
  geom_point(position = "dodge") + 
  # geom_errorbar(aes(xmin = min, xmax = max)) +
  theme_minimal() +
  theme(legend.position = "bottom",
  ...
  # scale_x_continuous(breaks=seq(0,max(plot_data$wtd_avg_income, na.rm=T), 10000)) +
  # scale_colour_manual(values=colours) +
  # geom_text_repel(aes(wtd_avg_income_proportion, Majorgroupcode_labelled, color = outsourcing_group, label=paste0("n=",n)), size=3, force=.5, box.padding=0.5) +
  geom_text(inherit.aes=F,data=annotation_df, aes(x=ypos, y=Majorgroupcode_labelled, label = paste0("N = ", N)), hjust=1) +
  # guides(size=FALSE) + # remove size legend as gauging size is difficult 
  xlab("Percentage of non-outsourced pay") + ylab("Major occupation") +
  labs(caption = "Note: n in indicates the size of the outsourced workforce in the sector") +
  scale_colour_manual(values=better_colours) 
...
write_csv(sector_summary_pay, file="../outputs/data/mgc_summary_pay_diff_o-group.csv")
```

Note that the high pay in high indicators group for skilled trades occupations is driven by only 4 data points:

```{r}
test <- data %>%
  filter(income_drop_all == 0 & !is.na(income_annual_all)) %>%
  filter(Majorgroupcode_labelled == "Skilled trades occupations") %>%
...
annotation_df <- test %>%
  ...
  # group_by(UnitOccupation_labelled) %>%
    mutate(
      # N = sum(n),
    ypos = max(test$wtd_avg_income, na.rm=T) * 1.2
  ) 

ggplot(test, aes(wtd_avg_income, UnitOccupation_labelled, colour = outsourcing_group)) +
  geom_point() +
...
write_csv(mgc_summary_paysplit, file="../outputs/data/mgc_summary_paysplit_group.csv")
...
write_csv(unit_occ_summary, file="../outputs/data/unit_occ_summary_agency_group.csv")
```

For the 'likely agency' subgroup, `r round(unit_occ_summary %>% filter(rank == 10) %>%   pull(cum_perc),0)`% of workers are located in the top 10 most common unit occupations. The top 15 unit occupations capture `r round(unit_occ_summary %>% filter(rank == 15) %>%pull(cum_perc),0)`% of the workforce, and `r round(unit_occ_summary %>% filter(rank == 30) %>%pull(cum_perc),0)`% of the workforce are captured in 30 unit occupations (out of a total of `r unit_occ_summary %>%  summarise(max(rank)) %>%  pull()`).[^39]

[^39]: [outputs/data/unit_occ_summary_agency_group.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/unit_occ_summary_agency_group.csv)
...
[^41]: [outputs/data/managers_unitoccs_summary_pay.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/managers_unitoccs_summary_pay.csv)
...
Breaking down by outsourcing group shows that most workers are outsourced in the 'Functional managers and directors' group, although a small number are likely agency.[^42]

[^42]: [outputs/data/o-group_managers_unitoccs_summary_pay.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/o-group_managers_unitoccs_summary_pay.csv)
...
The plot below indicates there are many unit occupations in this major occupation group where outsourced workers are paid less. The most notable of these are 'Information and Technology Technicians', 'Business Associate Professionals', and 'Sales, Marketing and Related Associate Professionals'.[^43]

[^43]: [outputs/data/assoc-prof_unitoccs_summary_pay.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/assoc-prof_unitoccs_summary_pay.csv)
...
The plot below shows that the pattern above is mainly driven by people in the 'outsourced' group.[^44]

[^44]: [outputs/data/o-group_assoc-prof_unitoccs_summary_pay.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/o-group_assoc-prof_unitoccs_summary_pay.csv)
...
[^45]: [outputs/data/prof_unitoccs_summary_pay.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/prof_unitoccs_summary_pay.csv)
...
Examining by outsourcing group indicates that the outsourced group is the main driver of lower pay in the identified unit occupations.[^46]

[^46]: [outputs/data/o-group_prof_unitoccs_summary_pay.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/o-group_prof_unitoccs_summary_pay.csv)
...
For this major occupation group, the notable differences in pay are in 'Caring Personal services and 'Teaching and Childcare Support Occupations'.[^47]

[^47]: [outputs/data/caring_unitoccs_summary_pay.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/caring_unitoccs_summary_pay.csv)
...
Breaking down by outsourcing group shows that the outsourced and likely agency groups are paid less than non-outsourced workers. In contrast, for 'Caring Personal Services', workers in the 'high indicators' group tend to be paid more than non-outrsourced people (though note n = 13).[^48]

[^48]: [outputs/data/o-group_caring_unitoccs_summary_pay.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/o-group_caring_unitoccs_summary_pay.csv)
...
Again, the outsourced group is the main driver of lower pay.[^50]

[^50]: [outputs/data/o-group_process_unitoccs_summary_pay.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/o-group_process_unitoccs_summary_pay.csv)
...
<!-- # base <- lm(income_annual_all ~ Age + Gender + Ethnicity_collapsed + BORNUK_labelled + Region + outsourcing_status, test_data, weights = NatRepemployees) -->

<!-- # summary(base) -->

<!-- #  -->

<!-- # # random effects improve it -->
...
<!--   filter(!is.na(Majorgroupcode_labelled)) -->

<!-- base <- lm(income_annual_all ~ Age + Gender + Ethnicity_collapsed + BORNUK_labelled + Region + outsourcing_status, test_data, weights = NatRepemployees) -->

<!-- summary(base) -->

<!-- base_2 <- lmerTest::lmer(income_annual_all ~ Age + Gender + Ethnicity_collapsed + BORNUK_labelled + Region + outsourcing_status + (1|Majorgroupcode_labelled), test_data, weights = NatRepemployees) -->

<!-- summary(base_2) -->

<!-- mod <- lmerTest::lmer(income_annual_all ~ Age + Gender + Ethnicity_collapsed + BORNUK_labelled + Region + outsourcing_status + (outsourcing_status|Majorgroupcode_labelled), test_data, weights = NatRepemployees) -->
...
<!-- base_2 <- lmerTest::lmer(income_annual_all ~ Age + Gender + Ethnicity_collapsed + BORNUK_labelled + Region + outsourcing_status + (1|Majorgroupcode_labelled/UnitOccupation_labelled), test_data, weights = NatRepemployees) -->

<!-- summary(base_2) -->

<!-- mod <- lmerTest::lmer(income_annual_all ~ Age + Gender + Ethnicity_collapsed + BORNUK_labelled + Region + outsourcing_status + (outsourcing_status|Majorgroupcode_labelled/UnitOcc_short), test_data, weights = NatRepemployees) -->
...
<!-- random_effects <- broom.mixed::tidy(mod, effects="ran_vals") -->

<!-- ``` -->

<!-- ```{r} -->

<!-- # Create a scatterplot of random effects -->

<!-- ggplot(random_effects, aes(x = term, y = estimate, color = level)) + -->
...
<!--     labs(title = "Random Effects by Grouping Variable", -->
...
<!-- mod_2 <- lmerTest::lmer(income_annual_all ~ Age + Gender + Ethnicity_collapsed + BORNUK_labelled + Region + outsourcing_status + (outsourcing_status|Majorgroupcode_labelled), test_data, weights = NatRepemployees) -->
...
<!-- # majorgroupcode looks like the better fit to the data. -->

<!-- ``` -->

<!-- ```{r} -->

<!-- mod_2 <- lmerTest::lmer(income_annual_all ~ Age + Gender + Ethnicity_collapsed + BORNUK_labelled + Region + outsourcing_status + (outsourcing_status|MajorsubgroupOccupation_labelled), test_data, weights = NatRepemployees) -->
...
<!-- sjPlot::plot_model(mod_2, type = "re", show.values = F) -->

<!-- ``` -->

<!-- ```{r} -->

<!-- mod <- lm(income_annual_all ~ Age + Gender + Ethnicity_collapsed + BORNUK_labelled + Region + outsourcing_group + SectorName_short, test_data, weights = NatRepemployees) -->
...
```{r include=FALSE}
ggplot(income_data, aes(outsourcing_status, income_annual_all, colour = SectorName_short, group = SectorName_short)) +
  geom_point() +
  geom_line() +
  guides(color="none")
```
