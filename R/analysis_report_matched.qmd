...
income_data2 <- income_data %>%
  drop_na(c(income_annual_all, Age,Gender,Has_Degree, Ethnicity_collapsed, Region, outsourcing_status, BORNUK_labelled))
# Annual income
# Intercept only
mod_base <- lm(income_annual_all ~ 1, income_data2, weights = NatRepemployees)
# H1
mod_annual <- lm(income_annual_all ~ Age + Gender + Has_Degree + Ethnicity_collapsed + Region + outsourcing_status + BORNUK_labelled, income_data2, weights = NatRepemployees)
...
# Weekly income
# Intercept only
mod_base <- lm(income_weekly_all ~ 1, income_data2, weights = NatRepemployees)
# H1
mod_weekly <- lm(income_weekly_all ~ Age + Gender + Has_Degree + Ethnicity_collapsed + Region + outsourcing_status + BORNUK_labelled, income_data2, weights = NatRepemployees)
summary(mod_weekly)
...
# write_csv(major_occ_summary_pay, file="../outputs/data/major_subgroup_across_sectors_occupation_summary_pay_hourly.csv")
...
guides(size=FALSE) + # remove size legend as gauging size is difficult 
xlab("Weighted average hourly income") + ylab("Major sub group occupation") +
labs(caption = "Size of bubble represents the size of the respective workforce within the occupation") +
  ggtitle("All sectors")
...
test <- data['NatRepemployees']

model_data <- mod2[["model"]] |> 
  rename(
    NatRepemployees = `(weights)`
  )
...
coefs <- extract_glm_coefs(mod2, only_sig=F)
...
design <- svydesign(
  ids = ~1,                                # Assuming no clustering
  weights = ~NatRepemployees,             # Your national survey weights
  data = data
)

# Step 2: Fit the model with quasibinomial family
mod_svy1 <- svyglm(
  outsourcing_status ~ Ethnicity_binary,
  design = design,
...
anova(mod_svy1,mod_svy2)
anova(mod_svy1,mod_svy2, method="Wald")

coefs2 <- extract_glm_coefs(mod_svy2, only_sig=F)

```

```{r}
regTermTest(mod_svy2, ~ Age + Gender + Has_Degree + Region + BORNUK_labelled)
regTermTest(mod_svy2, ~ Age + Gender + Has_Degree + Region + BORNUK_labelled, method = "Wald")
...
design <- svydesign(
  ids = ~1,  # Assuming no clustering
  weights = ~NatRepemployees,  # Survey weights
  data = data
)
...
tjur <- model_data %>%
  mutate(pred = predict(mod_svy, type = "response")) %>%
  group_by(outsourcing_status) %>%
  summarise(weighted_mean = weighted.mean(pred, NatRepemployees)) %>%
  summarise(tjur_r2 = diff(weighted_mean)) %>%
  pull()
