...
readr::write_csv(data, file = "../outputs/data/whole_dataset.csv")
```
...
### Pay^[Data file: outputs/data/income_stats.csv]
...
test_2 <- data %>%
  ...
  select(c(SectorName, `Outsourced workforce`))
...
Another way of looking at this is to calculate for each ethnicity, the proportion of workers in each outsourcing group. Doing so yields the plot below.^[Data file: outputs/data/ethnicity_stats_2.csv]
...
### Gender^[Data file: outputs/data/sex_statistics.csv]

```{r}
sex_statistics <- data %>%
  # get values of labels
  mutate_all(haven::as_factor) %>%
  group_by(outsourcing_status, Sex) %>%
  summarise(
    Frequency = n()
  ) %>%
  mutate(
...
readr::write_csv(sex_statistics, file="../outputs/data/sex_statistics.csv")
```

```{r}
sex_statistics %>%
  ggplot(., aes(outsourcing_status, Percentage, fill = Sex)) +
  geom_col(colour="black") +
  annotate("text", x = sex_statistics$outsourcing_status, y = 75, label = paste0("n=", sex_statistics$Frequency)) +
  coord_flip() +
...
In terms of sex, the outsourced group has a larger proportion of males (`r round(sex_statistics[which(sex_statistics$outsourcing_status == "Outsourced" & sex_statistics$Sex == "Male"),"Percentage"], 2)`% compared to `r round(sex_statistics[which(sex_statistics$outsourcing_status == "Outsourced" & sex_statistics$Sex == "Male"),"Percentage"], 2)`%.

### Region^[Data file: outputs/data/region_statistics.csv]
...
As shown in the table below, the median age of the outsourced group is `r age_statistics[which(age_statistics$outsourcing_status=="Outsourced"),"median"]` , compared to `r age_statistics[which(age_statistics$outsourcing_status=="Not outsourced"),"median"]` for the not outsourced group.^[Data file: outputs/data/age_stats.csv]
