...
[^6]: Data file: outputs/data/sex_statistics.csv
...
```{r sex_inferential, output=FALSE}
categories <- as.vector(unique(haven::as_factor(data$Sex)))
...
summary_table <- data %>%
  ...
  mutate(
    Sex = forcats::fct_collapse(as.character(Sex),
                                      "Male" = "Male",
...
ttest <- t.test(data_labelled[which(data_labelled$outsourcing_status=="Outsourced"),"Age"], 
       data_labelled[which(data_labelled$outsourcing_status=="Not outsourced"),"Age"]
)
