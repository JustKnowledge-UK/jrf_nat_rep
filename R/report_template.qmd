...
ttest <-  sjstats::weighted_ttest(income_annual_all ~ outsourcing_status + NatRepemployees, temp_data)
#cohens_d(ttest)
ttest
```
...
Building on above, explore each sector where outsourced people are more likely to be in the low paid group to see the pay distribution.

```{r}
sectors <- as.character(sig_cons$SectorName_short)

for(sector in sectors){
  plot <- data %>% 
    filter(., SectorName_short == sector & income_drop_all == 0 & outsourcing_status == "Outsourced") %>%
    ggplot(., aes(income_annual_all, fill = income_group)) +
    geom_density(alpha=0.3) +
    ggtitle(sector)
...
Below is the same breakdown but with *collapsed* ethnicity categories[^19]

[^19]: [outputs/data/ethnicity_stats_2.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/ethnicity_stats_2.csv)
...
[^20]: [outputs/data/ethnicity_stats_paysplit_1.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/ethnicity_stats_paysplit_1.csv) and [outputs/data/ethnicity_stats_paysplit_collapsed.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/ethnicity_stats_paysplit_collapsed.csv)
...
#### Outsourcing, ethnicity, and income group

TO BE COMPLETED - confirm appropriate method for post-hoc contrasts

```{r}
# mod <- glm(income_group ~ Age + BORNUK_labelled + Gender + Region + outsourcing_status*Ethnicity_collapsed, data, family="quasibinomial", weights = NatRepemployees)
# summary(mod)
```

```{r}
# ems <- emmeans(mod, specs = "outsourcing_status", by ="Ethnicity_collapsed")
# contrast(ems, "pairwise", adjust="Tukey")

```

Exploring ethnicity, outsourcing status and income group indicates that the mixed other group is more likely to be

```{r}
# sjPlot::plot_model(mod, type = "pred", legend.title = "", terms = c("outsourcing_status","Ethnicity_collapsed"), dodge = 0.6) +
#   coord_flip()
```

```{r}
# mod <- glm(income_group ~ Age + BORNUK_labelled + Gender + Region + outsourcing_group*Ethnicity_collapsed, data, family="quasibinomial", weights = NatRepemployees)
# summary(mod)

```

```{r}
# ems <- emmeans(mod, specs = "outsourcing_group", by ="Ethnicity_collapsed")
# contrast(ems, "pairwise", adjust="Tukey")

```

```{r}
# sjPlot::plot_model(mod, type = "pred", legend.title = "", terms = c("outsourcing_group","Ethnicity_collapsed"), dodge = 0.6) +
#   coord_flip()
```
...
[^21]: [outputs/data/ethnicity_stats_2.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/ethnicity_stats_2.csv)
...
### Ethnicity repeat: Outsourcing groups[^22]

[^22]: [outputs/data/ethnicity_stats_outsourcing_groups_1.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/ethnicity_stats_outsourcing_groups_1.csv)
...
```{r}
mod <- multinom(outsourcing_group ~ Ethnicity_collapsed, data, family="quasibinomial", weights=NatRepemployees)
#summary(mod)
...
The result of testing outsourcing group membership by ethnicity indicates the following:

Compared to White workers,

White other workers are:

-   `r round(sig_ors['Outsourced','Ethnicity_collapsedWhite other'],2)` times more likely to be outsourced
-   No more or less likely to be likely agency
-   No more or less likely to be high indicators

Black Caribbean workers are:

-   No more or less likely to be outsourced
-   No more or less likely to be likely agency
-   No more or less likely to be high indicators

Black African workers are:

-   `r round(sig_ors['Outsourced','Ethnicity_collapsedBlack African'],2)` times more likely to be outsourced
-   `r round(sig_ors['Likely agency','Ethnicity_collapsedBlack African'],2)` times more likely to be likely agency
-   `r round(sig_ors['High indicators','Ethnicity_collapsedBlack African'],2)` times more likely to be high indicators

Mixed other workers are:

-   `r round(sig_ors['Outsourced','Ethnicity_collapsedMixed other'],2)` times more likely to be outsourced
-   `r round(sig_ors['Likely agency','Ethnicity_collapsedMixed other'],2)` times more likely to be likely agency
-   No more or less likely to be high indicators

South Asian workers are:

-   `r round(sig_ors['Outsourced','Ethnicity_collapsedSouth Asian'],2)` times more likely to be outsourced
-   `r round(sig_ors['Likely agency','Ethnicity_collapsedSouth Asian'],2)` times more likely to be likely agency
-   `r round(sig_ors['High indicators','Ethnicity_collapsedSouth Asian'],2)` times more likely to be high indicators

East Asian workers are:
...
-   `r round(sig_ors['Outsourced','Ethnicity_collapsedOther'],2)` times more likely to be outsourced
-   `r round(sig_ors['Likely agency','Ethnicity_collapsedOther'],2)` times more likely to be likely agency
-   No more or less likely to be high indicators

Black other workers are:

-   `r round(sig_ors['Outsourced','Ethnicity_collapsedBlack other'],2)` times more likely to be outsourced
-   No more or less likely to be likely agency
-   No more or less likely to be high indicators
...
-   No more or less likely to be outsourced
-   `r round(sig_ors['Likely agency','Ethnicity_collapsedArab'],2)` times more likely to be likely agency
-   `r round(sig_ors['High indicators','Ethnicity_collapsedArab'],2)` times more likely to be high indicators

::: callout-note
(Note these stats should be double checked for reliability)
:::
...
[^24]: [outputs/data/bornuk_stats_paysplit_1.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/bornuk_stats_paysplit_1.csv)
...
### Interaction: Ethnicity and arrival in UK[^25]

[^25]: [outputs/data/interaction_ethnicity_arrival_in_UK_stats.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/interaction_ethnicity_arrival_in_UK_stats.csv)
...
[^26]: [outputs/data/gender_statistics.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/gender_statistics.csv)
...
[^27]: [outputs/data/gender_stats_paysplit_1.csv](https://github.com/Project-X-UK/jrf_nat_rep/blob/main/outputs/data/gender_stats_paysplit_1.csv)
