...
ci_results2 <- ci_results %>%
  pivot_longer(
    ...
    names_to = c("income_level", "ci_bound"),
    names_pattern = "prob_(.*)_ci_(.*)",
    values_to = "ci_value"
  ) %>%
  pivot_wider(
    names_from = ci_bound,
...
ggplot(plot_data, aes(y = Ethnicity_collapsed, x = probability,
...
Comparing ethnicities within outsourcing status does not add to the picture much,
...
# Plot predicted probabilities
ggplot(plot_data, aes(y = Ethnicity_collapsed_disaggregated, x = probability,
                      colour = outsourcing_status)) +
  geom_point(position = position_nudge(y = plot_data$nudge)) +
  geom_errorbar(position = position_nudge(y = plot_data$nudge), aes(xmin = ci_lower, xmax = ci_upper), width = 0.2) +
  facet_grid(rows = vars(income_level)) +
...
# Load data
experiential_data <- read_csv("../Experiential/data/2025-06-30 - clean_data_jrf_experiential.csv")
...
negative_outcomes_plot <- ggplot(negative_outcomes_summary, 
  ...
  geom_col(aes(fill = negative_outcomes_category), alpha = 0.8) +
  scale_fill_manual(values = c("#27ae60", "#f39c12", "#e67e22", "#e74c3c"), guide = "none") +
  geom_text(aes(label = paste0(Percentage, "%")), 
...
rights_experiential_data <- experiential_data %>%
  ...
  dplyr::select(
    ...
    OutsourcedNonOL, BORNUK, income_group
  )
...
### Multiple Rights Violations by Income Level

This analysis examines whether low-paid outsourced workers are more likely to report multiple workplace rights violations, following the same analytical approach as the benefits/downsides analysis.

```{r rights-violations-by-income}
#| fig-width: 12
#| fig-height: 8
#| out-width: "100%"
#| dpi: 300

# Create income-level analysis using existing prepared data
rights_by_income <- rights_experiential_data %>%
  # Filter out those without income data
  filter(!is.na(income_group)) %>%
  # Select only the rights violation numeric columns and add demographic info
  dplyr::select(RightsViolations_Paid_On_Time_numeric,
                RightsViolations_Paid_Correct_Amount_numeric,
                RightsViolations_Leave_Entitlement_numeric,
                RightsViolations_Holiday_Pay_numeric,
                ...
                income_group) %>%
  # Remove rows where all rights variables are NA
  filter(rowSums(is.na(dplyr::select(., -income_group))) < 7) %>%
  # Count negative responses per person (same as original analysis)
  mutate(
    num_neg = rowSums(dplyr::select(., -income_group) == -1, na.rm = TRUE)
  ) %>%
  # Categorise by number of negative experiences (same categories as original)
  mutate(neg_category = case_when(
    num_neg == 0 ~ "0",
    num_neg == 1 ~ "1",
    num_neg == 2 ~ "2",
  ...
  mutate(
    violations_category = case_when(
      num_neg == 0 ~ "No violations",
      num_neg == 1 ~ "1 violation",
      num_neg == 2 ~ "2 violations",
      num_neg == 3 ~ "3 violations",
      num_neg >= 4 ~ "4+ violations",
      TRUE ~ NA_character_
    )
  )

# Create summary by income group
rights_by_income_summary <- rights_by_income %>%
  count(income_group, violations_category) %>%
  ...
  ungroup() %>%
  filter(!is.na(violations_category))

# Order categories logically
rights_by_income_summary$violations_category <- factor(
  rights_by_income_summary$violations_category,
  levels = c("No violations", "1 violation", "2 violations", "3 violations", "4+ violations")
)

# Create stacked bar chart showing distribution by income group
rights_income_plot <- ggplot(rights_by_income_summary, 
                            aes(x = income_group, y = percentage, fill = violations_category)) +
  geom_col(position = "stack", width = 0.7) +
  geom_text(aes(label = ifelse(percentage >= 4, paste0(percentage, "%"), "")), 
            position = position_stack(vjust = 0.5), 
            size = 3.2, fontweight = "bold", color = "white") +
  labs(
    title = "Distribution of Rights Violations by Income Level",
    subtitle = "Percentage of workers experiencing different numbers of workplace rights violations",
    x = "Income Group",
    y = "Percentage of Workers",
    fill = "Number of Violations"
  ) +
  scale_fill_manual(values = c("#27ae60", "#3498db", "#f39c12", "#e67e22", "#e74c3c")) +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 14, face = "bold"),
    plot.subtitle = element_text(size = 12, color = "gray60"),
...
# Create summary statistics table by income group
rights_violations_summary_table <- rights_by_income %>%
  group_by(income_group) %>%
  summarise(
    n = n(),
    mean_violations = round(mean(num_neg, na.rm = TRUE), 2),
    median_violations = median(num_neg, na.rm = TRUE),
    pct_no_violations = round(mean(num_neg == 0, na.rm = TRUE) * 100, 1),
    pct_multiple_violations = round(mean(num_neg >= 2, na.rm = TRUE) * 100, 1),
    pct_severe_violations = round(mean(num_neg >= 4, na.rm = TRUE) * 100, 1),
    .groups = "drop"
  )

# Display summary table
rights_violations_summary_table %>%
  rename(
    "Income Group" = income_group,
    "N" = n,
    "Mean Violations" = mean_violations,
    "Median" = median_violations,
    "No Violations (%)" = pct_no_violations,
    "2+ Violations (%)" = pct_multiple_violations,
    "4+ Violations (%)" = pct_severe_violations
  ) %>%
  flextable() %>%
  theme_vanilla() %>%
  autofit()

# Create focused visualization for multiple violations (2+) by income level
multiple_violations_by_income <- rights_by_income %>%
  group_by(income_group) %>%
  summarise(
    total = n(),
    multiple_violations = sum(num_neg >= 2, na.rm = TRUE),
    percentage = round(multiple_violations / total * 100, 1),
    .groups = "drop"
  )

multiple_violations_plot <- ggplot(multiple_violations_by_income, 
                                  aes(x = income_group, y = percentage)) +
  geom_col(fill = c("#e74c3c", "#f39c12", "#27ae60"), alpha = 0.8, width = 0.6) +
  geom_text(aes(label = paste0(percentage, "%")), 
            vjust = -0.5, size = 4, fontweight = "bold") +
  labs(
    title = "Workers with Multiple Rights Violations by Income Level",
    subtitle = "Percentage experiencing 2 or more workplace rights violations",
    x = "Income Group",
    y = "Percentage of Workers"
  ) +
  theme_minimal() +
  ...
  scale_y_continuous(expand = expansion(mult = c(0, 0.1)), limits = c(0, max(multiple_violations_by_income$percentage) + 3))

multiple_violations_plot

# Statistical analysis
# Chi-square test for association between income group and multiple violations
rights_by_income_test <- rights_by_income %>%
  mutate(multiple_violations = num_neg >= 2)

violations_contingency_table <- table(rights_by_income_test$income_group, 
                                     rights_by_income_test$multiple_violations)

chi_test_rights <- chisq.test(violations_contingency_table)

# Calculate Cramér's V for effect size
cramers_v_rights <- sqrt(chi_test_rights$statistic / (sum(violations_contingency_table) * (min(dim(violations_contingency_table)) - 1)))

# Display test results
cat("Statistical Test for Association between Income Level and Multiple Rights Violations:\n")
cat("Chi-square statistic:", round(chi_test_rights$statistic, 3), "\n")
cat("Degrees of freedom:", chi_test_rights$parameter, "\n")
cat("p-value:", format.pval(chi_test_rights$p.value, digits = 3), "\n")
cat("Cramér's V (effect size):", round(cramers_v_rights, 3), "\n\n")

# Calculate exact percentages for interpretation
low_multiple <- rights_violations_summary_table$pct_multiple_violations[rights_violations_summary_table$income_group == "Low"]
mid_multiple <- rights_violations_summary_table$pct_multiple_violations[rights_violations_summary_table$income_group == "Mid"]  
high_multiple <- rights_violations_summary_table$pct_multiple_violations[rights_violations_summary_table$income_group == "High"]

low_mean <- rights_violations_summary_table$mean_violations[rights_violations_summary_table$income_group == "Low"]
mid_mean <- rights_violations_summary_table$mean_violations[rights_violations_summary_table$income_group == "Mid"]
high_mean <- rights_violations_summary_table$mean_violations[rights_violations_summary_table$income_group == "High"]

low_none <- rights_violations_summary_table$pct_no_violations[rights_violations_summary_table$income_group == "Low"]
high_none <- rights_violations_summary_table$pct_no_violations[rights_violations_summary_table$income_group == "High"]
```

The analysis reveals **clear income-related disparities in workplace rights violations** among outsourced workers. **Low-income workers face the highest burden of multiple violations**: `r low_multiple`% experience two or more rights violations compared to `r mid_multiple`% of mid-income workers and `r high_multiple`% of high-income workers.

**The income gradient in violations is pronounced**: low-income workers average `r low_mean` violations per person, compared to `r mid_mean` for mid-income and `r high_mean` for high-income workers. **Most concerning is that only `r low_none`% of low-income workers report no rights violations** compared to `r high_none`% of high-income workers, indicating that **the majority of low-paid outsourced workers experience at least one workplace rights violation**.

This pattern **directly mirrors the benefits/downsides analysis**, where low-paid workers also experienced more negative workplace outcomes. The findings suggest that **pay disadvantage in outsourced work is systematically associated with broader patterns of workplace rights violations** - low pay appears to be a marker of comprehensively disadvantaged working conditions rather than an isolated issue.

The statistical analysis confirms a significant association between income level and multiple rights violations (χ² = `r round(chi_test_rights$statistic, 2)`, p `r ifelse(chi_test_rights$p.value < 0.001, "< 0.001", paste("=", round(chi_test_rights$p.value, 3)))`, Cramér's V = `r round(cramers_v_rights, 3)`), providing strong evidence that **income level is a meaningful predictor of cumulative workplace rights violations among outsourced workers**.
