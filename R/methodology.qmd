...
A model including the ethnicity:outsourcing interaction term was not a significant improvement on a model without the interaction term, *F*(`r dfs[[1]]`, `r dfs[[2]]`) = `r f`, *p* `r p`. The table below shows the model coefficients.
...
labels <- c(
  ...
  'Migration: Arrived more than 30 years ago',
  'Migration: Prefer not to say',
  'Interaction: Outsourced x Non-White',
  "Interaction: Outsourced x Don't think of myself as any of these",
  "Interaction: Outsourced x Prefer not to say"
)

tab_model(mod_svy2, 
          pred.labels = labels,
          dv.labels = "Low income group",
          show.r2 = FALSE)
```
...
A model including the ethnicity:outsourcing interaction term significantly improved model fit compared to a model without the interaction term, *F*(`r dfs[[1]]`, `r dfs[[2]]`) = `r f`, *p* `r p`. The table below shows the model coefficients.
...
labels <- c(
  ...
  'Migration: Arrived more than 30 years ago',
  'Migration: Prefer not to say',
  'Interaction: Outsourced x Arab/British Arab',
  'Interaction: Outsourced x Asian/Asian British',
  'Interaction: Outsourced x Black/African/Caribbean/Black British',
  "Interaction: Outsourced x Don't think of myself as any of these",
  'Interaction: Outsourced x Mixed/Multiple ethnic group',
  'Interaction: Outsourced x Other ethnic group',
  'Interaction: Outsourced x Prefer not to say',
  'Interaction: Outsourced x White other'
)
...
```{r}
# reps <- 500
# # STEP 1: Create bootstrap replicate weights
# boot_design <- as.svrepdesign(design, type = "bootstrap", replicates = reps)
# 
# # Model 1: without interaction
# mod_svy_boot_no_inter <- svy_vglm(
#   income_group ~ outsourcing_status + Ethnicity_collapsed + Age + Gender + Has_Degree + Region + BORNUK_labelled,
#   design = boot_design,
#   family = multinomial(refLevel = "Low")
# )
...
# mod_svy_boot_inter <- svy_vglm(
#   income_group ~ outsourcing_status + Ethnicity_collapsed + Age + Gender + Has_Degree + Region + BORNUK_labelled + outsourcing_status:Ethnicity_collapsed,
#   design = boot_design,
#   family = multinomial(refLevel = "Low")
# )
# 
# # Extract coefficients and vcov from model with interaction
# coefs <- coef(mod_svy_boot_inter)
# vcovs <- vcov(mod_svy_boot_inter)
# 
# # Get names of interaction terms
# interaction_names <- grep("outsourcing_status.*:Ethnicity_collapsed", names(coefs), value = TRUE)
# 
# # Subset coefficients and vcov for interaction terms
# L <- coefs[interaction_names]
# V <- vcovs[interaction_names, interaction_names]
# 
# # Wald statistic
# W <- t(L) %*% solve(V) %*% L
# df <- length(L)
# p_value <- pchisq(W, df, lower.tail = FALSE)
# 
# # Print results
...
# tidy_mod <- tidy.svyVGAM(mod_svy_boot_inter, exponentiate = TRUE, conf.int = TRUE)
# 
# tidy_mod %>% 
#   dplyr::select(-y.level) %>% 
#   rename(income_group = term) %>% 
...
filepath <- here("Data","bootstrap1.rds")
if(!file.exists(filepath)){
# Run bootstrap (this will take some time)
  cat("Running bootstrap... this may take a few minutes\n")
  boot_samples <- bootstrap_predictions(n_boot = 500)  # Start with fewer for testing
...
plan(multisession, workers = 14)  # Or plan(multisession, workers = 4)
...
cat(paste0("Done in ", difftime(t2, t1, units = "secs"), " seconds"))
# Run or load
# filepath <- here("data", "bootstrap2_test.rds")
# if (!file.exists(filepath)) {
#   cat("Running bootstrap... this may take a few minutes\n")
#   boot_samples <- bootstrap_predictions(n_boot = 10)
#   saveRDS(boot_samples, filepath)
# } else {
#   cat("Loading previous bootstrap\n")
  ...
  for(eth in names(ethnicity_groups)) {
    ...
    if(nrow(group_data) >= 2) {
      ...
      if(length(outsourcing_levels) == 2) {
        ...
        prob_diff <- group_data[group_data$outsourcing_status == outsourcing_levels[2], prob_col] - 
        ...
        if(length(boot_diffs) > 2) {  # Need sufficient bootstrap samples
          ci_lower <- quantile(boot_diffs, 0.025, na.rm = TRUE)
          ci_upper <- quantile(boot_diffs, 0.975, na.rm = TRUE)
          
          # Approximate p-value (proportion of bootstrap samples crossing zero)
          p_value <- 2 * min(mean(boot_diffs > 0, na.rm = TRUE), 
...
if(nrow(difference_results) > 0) {
  ggplot(difference_results, aes(x = ethnicity, y = prob_difference)) +
    geom_point() +
    ...
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
    labs(title = "Differences in Predicted Probabilities",
         subtitle = "Error bars show 95% bootstrap confidence intervals",
         x = "Ethnicity", y = "Probability Difference")
}
```

### Ethnicity within outsourcing
  ...
  for(out_group in names(outsourcing_groups)) {
    ...
    for(eth in other_ethnicities) {
      ...
      if(nrow(comp_data) > 0) {
        ...
        prob_diff <- comp_data[[prob_col]] - ref_data[[prob_col]]
...
```{r}
if(nrow(ethnicity_diff_results) > 0) {
  ggplot(ethnicity_diff_results, aes(x = ethnicity, y = prob_difference)) +
    geom_point(size = 2) +
    geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), width = 0.2) +
    geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
    ...
    labs(
      ...
      x = "Ethnic Group (vs White British)",
...
```{r}
# Test just for Mid
mid_results <- calculate_differences_with_ci(pred_results, boot_samples, "Mid")
print("Mid results:")
print(mid_results)
```
...
summary_table <- pred_results %>%
  pivot_longer(cols = starts_with("prob_"), 
               names_to = "income_level", 
               values_to = "probability") %>%
  mutate(income_level = gsub("prob_", "", income_level)) %>%
  pivot_wider(names_from = outsourcing_status, 
              values_from = probability) %>%
  arrange(Ethnicity_collapsed, income_level)
...
### end testing
```{r}
head(predict(mod_svy2$fit,type="response"))
```
...
emm <- emmeans(mod_svy2, ~ outsourcing_status | Ethnicity_collapsed, 
               mode = "prob")  # for probabilities

# Pairwise comparisons within each ethnicity group
pairs_within_ethnicity <- pairs(emm, by = "Ethnicity_collapsed")

# Pairwise comparisons within each outsourcing status
pairs_within_outsourcing <- pairs(emm, by = "outsourcing_status")

# View results
summary(pairs_within_ethnicity, adjust = "bonferroni")
summary(pairs_within_outsourcing, adjust = "bonferroni")
```


```{r}
# Create new data for prediction
# Get levels of interaction factors
# outsourcing_levels <- levels(design$variables$outsourcing_status)
# ethnicity_levels <- levels(design$variables$Ethnicity_collapsed)
# 
# # Fix covariates at their means or reference levels:
# mean_age <- mean(design$variables$Age, na.rm = TRUE)
# mode_gender <- names(sort(table(design$variables$Gender), decreasing = TRUE))[1]
# mode_degree <- names(sort(table(design$variables$Has_Degree), decreasing = TRUE))[1]
# mode_region <- names(sort(table(design$variables$Region), decreasing = TRUE))[1]
# mode_BORNUK <- names(sort(table(design$variables$BORNUK_labelled), decreasing = TRUE))[1]
# 
# newdata <- expand.grid(
#   outsourcing_status = outsourcing_levels,
#   Ethnicity_collapsed = ethnicity_levels,
#   Age = mean_age,
#   Gender = mode_gender,
#   Has_Degree = mode_degree,
#   Region = mode_region,
#   BORNUK_labelled = mode_BORNUK
# )
# 
...
# pred_array <- array(NA, dim = c(nrow(newdata), length(levels(design$variables$income_group)), B),
#                     dimnames = list(NULL, levels(design$variables$income_group), NULL))
# 
# for (b in 1:B) {
#   # Bootstrap replicate survey design (example for bootstrap replicates)
#   boot_design <- as.svrepdesign(design, type = "bootstrap", replicates = 1)
# 
#   # Fit model to bootstrap design
#   mod_boot <- svyVGAM::svy_vglm(
#     income_group ~ outsourcing_status + Ethnicity_collapsed + Age + Gender + Has_Degree + Region + BORNUK_labelled + outsourcing_status:Ethnicity_collapsed,
#     design = boot_design,
#     family = multinomial(refLevel = "Low")
#   )
#   
#   # Extract the underlying vglm object
#   mod_boot_vglm <- mod_boot$fit
#   
#   # Predict using the underlying vglm model
#   preds <- predict(mod_boot_vglm, newdata = newdata, type = "response")
#   
#   pred_array[, , b] <- preds
# }
# 
...
# 
# results <- as_tibble(pred_mean) %>%
#   mutate(row = 1:nrow(pred_mean)) %>%
#   pivot_longer(cols = -row, names_to = "income_group", values_to = "mean_prob") %>%
#   mutate(
#     lower = as.vector(pred_lower),
#     upper = as.vector(pred_upper),
#     outsourcing_status = rep(newdata$outsourcing_status, each = ncol(pred_mean)),
#     Ethnicity_collapsed = rep(newdata$Ethnicity_collapsed, each = ncol(pred_mean))
#   )
# 
# 
# 
# ggplot(results, aes(x = outsourcing_status, y = mean_prob, fill = income_group)) +
#   geom_col(position = "dodge") +
#   geom_errorbar(aes(ymin = lower, ymax = upper), position = position_dodge(width = 0.9), width = 0.2) +
#   facet_wrap(~ Ethnicity_collapsed) +
#   labs(title = "Bootstrap Predicted Probabilities with 95% CI",
#        y = "Predicted Probability", x = "Outsourcing Status") +
#   theme_minimal()
# 
...
# 
# # Extract the underlying vglm object
# mod_vglm <- mod_svy2$fit
# 
# # Predict using the underlying vglm model
# preds <- predict(mod_vglm, newdata = newdata, type = "response")
# 
# pred_array[, , b] <- preds
# 
# 
...
# # attempt 2
# B <- 500
# boot_design <- as.svrepdesign(design, type = "bootstrap", replicates = B)
# 
# mod_boot <- svyVGAM::svy_vglm(
#   income_group ~ outsourcing_status + Ethnicity_collapsed + Age + Gender + Has_Degree + Region + BORNUK_labelled + outsourcing_status:Ethnicity_collapsed,
#   design = boot_design,
...
# )
# 
# outsourcing_levels <- levels(mod_data$outsourcing_status)
# ethnicity_levels <- levels(mod_data$Ethnicity_collapsed)
# 
# mean_age <- mean(mod_data$Age, na.rm = TRUE)
# mode_gender <- names(sort(table(mod_data$Gender), decreasing = TRUE))[1]
# mode_degree <- names(sort(table(mod_data$Has_Degree), decreasing = TRUE))[1]
# mode_region <- names(sort(table(mod_data$Region), decreasing = TRUE))[1]
# mode_BORNUK <- names(sort(table(mod_data$BORNUK_labelled), decreasing = TRUE))[1]
# 
# newdata <- expand.grid(
#   outsourcing_status = outsourcing_levels,
#   Ethnicity_collapsed = ethnicity_levels,
...
# )
# 
# # Predict probabilities across replicates
# predictions <- predict(mod_boot, newdata = newdata, type = "response", se = TRUE)
# 
# # Extract point estimates and standard errors
# predicted_probs <- predictions$fit  # matrix: rows = newdata rows, columns = income_group levels
# predicted_se <- predictions$se.fit
# 
# # Compute 95% CIs
# lower <- predicted_probs - 1.96 * predicted_se
# upper <- predicted_probs + 1.96 * predicted_se
# 
# # Put into a tidy data frame
# results <- as.data.frame(predicted_probs)
# colnames(results) <- colnames(predicted_probs)
# 
# results <- results %>%
#   mutate(
#     outsourcing_status = newdata$outsourcing_status,
#     Ethnicity_collapsed = newdata$Ethnicity_collapsed
#   ) %>%
#   pivot_longer(cols = starts_with("Low"):last_col(),
...
# results <- results %>%
#   mutate(
#     se = as.vector(predicted_se),
#     lower = mean_prob - 1.96 * se,
#     upper = mean_prob + 1.96 * se
#   )
# 
# # Filter rows for Ethnicity == "White" and two outsourcing statuses
# contrast_data <- results %>%
#   filter(Ethnicity_collapsed == "White" & income_group == "High")
# 
# # Extract values
# p_outsourced <- contrast_data %>% filter(outsourcing_status == "Outsourced") %>% pull(mean_prob)
# p_not_outsourced <- contrast_data %>% filter(outsourcing_status == "Not_Outsourced") %>% pull(mean_prob)
# 
# se_outsourced <- contrast_data %>% filter(outsourcing_status == "Outsourced") %>% pull(se)
# se_not_outsourced <- contrast_data %>% filter(outsourcing_status == "Not_Outsourced") %>% pull(se)
# 
# # Compute difference and CI
# diff <- p_outsourced - p_not_outsourced
# se_diff <- sqrt(se_outsourced^2 + se_not_outsourced^2)
# 
...
# # Show result
# data.frame(
#   contrast = "Outsourced - Not Outsourced",
#   income_group = "High",
#   diff = diff,
#   se = se_diff,
#   lower = lower_diff,
...
```{r}
# Extract all coefficients and vcov
# coefs <- coef(mod_svy2)
# vcovs <- vcov(mod_svy2)
# 
# # Create contrast matrix for specific comparisons
# # Example: Compare outsourcing effects within each ethnicity
# contrast_matrix <- matrix(0, nrow = 1, ncol = length(coefs))
# 
# # You'll need to identify which coefficients correspond to your contrasts
# # This depends on your factor levels and model parameterization
# colnames(contrast_matrix) <- names(coefs)
# 
# # Set up your specific contrast (example)
# # contrast_matrix[1, "outsourcing_statusYes"] <- 1
# # contrast_matrix[1, "outsourcing_statusYes:Ethnicity_collapsedAsian"] <- 1
# 
# # Compute Wald test for this contrast
# contrast_est <- contrast_matrix %*% coefs
# contrast_var <- contrast_matrix %*% vcovs %*% t(contrast_matrix)
# wald_stat <- contrast_est^2 / contrast_var
# p_value <- pchisq(wald_stat, df = 1, lower.tail = FALSE)
```

```{r}
# Pairwise - outsorucig within ethnictiy

# Assume pred_array from previous bootstrap loop exists:
# dim(pred_array) = [nrow(newdata), n_outcome, B]

# Helper to get index rows for each group combo
# get_rows <- function(outsource_val, eth_val) {
#   which(newdata$outsourcing_status == outsource_val & newdata$Ethnicity_collapsed == eth_val)
# }
# 
# outsourcing_levels <- levels(newdata$outsourcing_status)
# ethnicity_levels <- levels(newdata$Ethnicity_collapsed)
# income_levels <- dimnames(pred_array)[[2]]
# B <- dim(pred_array)[3]
# 
# results_diff <- list()
# 
# for (eth in ethnicity_levels) {
#   for (income_cat in income_levels) {
#     # Get rows for each outsourcing level within this ethnicity
#     rows <- sapply(outsourcing_levels, get_rows, eth_val = eth)
#     
#     # Extract predicted probs for these groups from pred_array
#     # rows is a vector with one row per outsourcing_status
#     # For each bootstrap replicate:
#     diffs <- matrix(NA, nrow = B, ncol = length(outsourcing_levels)*(length(outsourcing_levels)-1)/2)
#     colnames(diffs) <- c()
#     
#     # Calculate pairwise differences between outsourcing statuses
#     # We'll do all pairs, but often just Yes vs No is enough
#     comp_idx <- 1
#     for (i in 1:(length(outsourcing_levels)-1)) {
#       for (j in (i+1):length(outsourcing_levels)) {
#         diffs[, comp_idx] <- pred_array[rows[i], income_cat, ] - pred_array[rows[j], income_cat, ]
#         colnames(diffs)[comp_idx] <- paste0(outsourcing_levels[i], " - ", outsourcing_levels[j])
#         comp_idx <- comp_idx + 1
#       }
#     }
#     
...
#       diff_mean <- mean(diffs[, k])
#       diff_lower <- quantile(diffs[, k], 0.025)
#       diff_upper <- quantile(diffs[, k], 0.975)
#       # Two-sided p-value from bootstrap: proportion of samples crossing zero
#       p_val <- 2 * min(mean(diffs[, k] > 0), mean(diffs[, k] < 0))
#       
#       results_diff[[length(results_diff) + 1]] <- data.frame(
#         Ethnicity_collapsed = eth,
#         income_group = income_cat,
#         comparison = colnames(diffs)[k],
#         mean_diff = diff_mean,
#         lower_CI = diff_lower,
#         upper_CI = diff_upper,
#         p_value = p_val
...
# }
# 
# results_diff_df <- bind_rows(results_diff)
# 
# # View results
...
# 
# 
# ggplot(results_diff_df, aes(x = comparison, y = mean_diff, ymin = lower_CI, ymax = upper_CI, color = income_group)) +
#   geom_pointrange(position = position_dodge(width = 0.5)) +
#   facet_wrap(~ Ethnicity_collapsed) +
#   geom_hline(yintercept = 0, linetype = "dashed") +
#   labs(title = "Differences in Predicted Probability by Outsourcing Status (within Ethnicity)",
#        y = "Difference in Predicted Probability",
#        x = "Comparison") +
#   theme_minimal()
...
```{r}
# ethnicity within outsroucign status
# results_diff2 <- list()
# 
# reference_ethnicity <- "White British"
# 
# for (outsource in outsourcing_levels) {
#   for (income_cat in income_levels) {
#     # Find the row index for the reference ethnicity within this outsourcing_status
#     ref_row <- which(newdata$Ethnicity_collapsed == reference_ethnicity & newdata$outsourcing_status == outsource)
#     
#     # Skip if reference row not found
#     if (length(ref_row) == 0) next
#     
#     # Get indices for all other ethnicities (excluding reference)
#     other_ethnicities <- setdiff(ethnicity_levels, reference_ethnicity)
#     other_rows <- sapply(other_ethnicities, function(eth) {
#       which(newdata$Ethnicity_collapsed == eth & newdata$outsourcing_status == outsource)
#     })
#     
#     # Initialize matrix for differences (B rows, one column per ethnicity compared to reference)
#     diffs <- matrix(NA, nrow = B, ncol = length(other_ethnicities))
#     colnames_vec <- character(length(other_ethnicities))
#     
#     for (i in seq_along(other_ethnicities)) {
#       diffs[, i] <- pred_array[other_rows[i], income_cat, ] - pred_array[ref_row, income_cat, ]
#       colnames_vec[i] <- paste0(other_ethnicities[i], " - ", reference_ethnicity)
#     }
#     colnames(diffs) <- colnames_vec
#     
#     # Summarize differences
#     for (k in seq_len(ncol(diffs))) {
#       diff_mean <- mean(diffs[, k])
#       diff_lower <- quantile(diffs[, k], 0.025)
#       diff_upper <- quantile(diffs[, k], 0.975)
#       p_val <- 2 * min(mean(diffs[, k] > 0), mean(diffs[, k] < 0))
#       
#       results_diff2[[length(results_diff2) + 1]] <- data.frame(
#         outsourcing_status = outsource,
#         income_group = income_cat,
#         comparison = colnames(diffs)[k],
#         mean_diff = diff_mean,
#         lower_CI = diff_lower,
...
# }
# 
# results_diff2_df <- dplyr::bind_rows(results_diff2)
# 
# 
# results_diff2_df$income_group <- factor(results_diff2_df$income_group, levels = c("Low", "Mid", "High"))
# 
# ggplot(results_diff2_df, aes(y = comparison, x = mean_diff, xmin = lower_CI, xmax = upper_CI, color = income_group)) +
#   geom_pointrange(position = position_dodge(width = 0.5)) +
#   facet_wrap(~ outsourcing_status) +
#   geom_vline(xintercept = 0, linetype = "dashed") +
#   labs(title = "Differences in Predicted Probability by Ethnicity (within Outsourcing Status)",
#        x = "Difference in Predicted Probability",
#        y = "Ethnicity Comparison") +
#   theme_minimal()
...
for (out in out_levels) {
  for (eth in eth_levels) {
    ...
    # Interaction term (only needed if BORNUK != ref)
    if (out != ref_out) {
      interaction_name <- paste0("outsourcing_status", out, ":", "Ethnicity_collapsed", eth)
      if (interaction_name %in% coefs) {
        vec[interaction_name] <- 1
      }
    }
...
cell_sizes <- mod_data %>%
  count(outsourcing_status, Ethnicity_collapsed) %>%
  rename(cell_n = n)

partA_df <- partA_df %>%
  separate(contrast, into = c("outsourcing_status", "ethnicity"), sep = ": ", remove = FALSE) %>%
  separate(ethnicity, into = c("test_ethn","ref_ethn"), sep = " vs ", remove = FALSE) %>%
  left_join(cell_sizes, by = c("outsourcing_status", "test_ethn" = "Ethnicity_collapsed")) %>%
  mutate(small_cell_flag = cell_n <= 10)
...
- Among people who are outsourced, people of "Mixed/Multiple ethnic group" are `r sig_partA[which(sig_partA$test_ethn == "Mixed/Multiple ethnic group"), "OR"] %>% round(2)` times more likely to be in the low income group than White British people.
-   Among people who are not outsourced, people of "Other ethnic group" are `r sig_partA[which(sig_partA$test_ethn == "Other ethnic group"), "OR"]` times as likely (i.e. `r 100 * (1-(sig_partA[which(sig_partA$test_ethn == "Other ethnic group"), "OR"]))`% less likely) than White British people to be in the low income group (**NB** sample size for this cell is 11).
...
ems_df %>%
  # filter(Ethnicity_collapsed_disaggregated %in% ethn_of_interest) %>%
  ggplot(aes(y = outsourcing_status, x = prob, colour = Ethnicity_collapsed)) +
  geom_point(position = pd, size = 2) +
  geom_errorbar(
    aes(xmin = asymp.LCL, xmax = asymp.UCL),
    width = 0.2,
...
for (eth in eth_levels) {
  for (out in out_levels) {
    ...
    if (eth != ref_eth) {
      ...
      # Catch flipped ordering, just in case
      if (!interaction_name %in% coefs) {
        alt_name <- paste0("Ethnicity_collapsed", eth, ":", "outsourcing_status", out)
        if (alt_name %in% coefs) interaction_name <- alt_name
      }
      if (interaction_name %in% coefs) {
        vec[interaction_name] <- 1
...
partB_df <- partB_df %>%
  ...
  left_join(cell_sizes, by = c("ethnicity" = "Ethnicity_collapsed", "test_out" = "outsourcing_status")) %>%
  mutate(small_cell_flag = cell_n <= 10)
...
1.  Among White British people, outsourced workers are `r sig_partB[which(sig_partB$ethnicity == "White British"), "OR"] %>% round(2)` times more likely to be in the low income group than non-outsourced workers.
2.  Among people of Mixed/Multiple ethnic groups, outsourced workers are `r sig_partB[which(sig_partB$ethnicity == "Mixed/Multiple ethnic group"), "OR"] %>% round(2)` times more likely to be in the low income group than non-outsourced workers.
...
ems_df %>%
  # filter(Ethnicity_collapsed_disaggregated %in% ethn_of_interest) %>%
  ggplot(aes(y = Ethnicity_collapsed, x = prob, colour = outsourcing_status)) +
  geom_point(position = pd, size = 2) +
  geom_errorbar(
    aes(xmin = asymp.LCL, xmax = asymp.UCL),
    width = 0.2,
...
labels <- c(
  ...
  'Migration: Arrived more than 30 years ago',
  'Migration: Prefer not to say',
  "Interaction: Outsourced x Irish",
  "Interaction: Outsourced x Gypsy or Irish Traveller",
  "Interaction: Outsourced x Roma",
  "Interaction: Outsourced x Any other White background",
  "Interaction: Outsourced x White and Black Caribbean",
  "Interaction: Outsourced x White and Black African",
  "Interaction: Outsourced x White and Asian",
  "Interaction: Outsourced x Any other Mixed/Multiple ethnic background",
  "Interaction: Outsourced x Indian",
  "Interaction: Outsourced x Pakistani",
  "Interaction: Outsourced x Bangladeshi",
  "Interaction: Outsourced x Chinese",
  "Interaction: Outsourced x Any other Asian background",
  "Interaction: Outsourced x African",
  "Interaction: Outsourced x Caribbean",
  "Interaction: Outsourced x Any other Black, Black British, or Caribbean background",
  "Interaction: Outsourced x Arab",
  "Interaction: Outsourced x Any other ethnic group",
  "Interaction: Outsourced x Don't think of myself as any of these",
  "Interaction: Outsourced x Prefer not to say"
  )
