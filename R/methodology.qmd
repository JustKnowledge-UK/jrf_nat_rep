...
drop <- unique(difference_results2[which(difference_results2$n < 20),1])

difference_results3 <- difference_results %>%
  filter(!(ethnicity %in% drop))
...
ggplot(difference_results3, aes(x = ethnicity, y = prob_ratio)) +
  geom_point() +
  ...
  labs(
    ...
    x = "Ethnic Group",
    y = "Ratio of Predicted Probabilities",
...
```{r}
if(nrow(ethnicity_diff_results) > 0) {
  ggplot(ethnicity_diff_results, aes(x = ethnicity, y = prob_ratio)) +
    geom_point(size = 2) +
    geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), width = 0.2) +
    geom_hline(yintercept = 1, linetype = "dashed", color = "red") +
