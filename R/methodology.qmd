...
mod <- glm(income_group ~ Age + outsourcing_status + Ethnicity_collapsed_disaggregated + Gender + Has_Degree + Region + BORNUK_binary, income_data, weights = NatRepemployees, family="quasibinomial")
summary(mod)
...
The value of Tjur's pseudo *R^2^* for this model is `r tjur %>% round(3)`. The table below shows the model coefficients. The result indicates that Age weakly but significantly influences whether a worker is in the low income group. For each year older a worker is, they are `r coefs %>% filter(variable=="Age") %>% pull(or) %>% round(3)` times more likely to be in the low income group. 
