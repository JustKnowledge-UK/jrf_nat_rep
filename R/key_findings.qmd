...
```{r}
mod <- glm(income_group ~ Age + Gender + Ethnicity_collapsed + BORNUK_labelled + Region + outsourcing_status, data, weights = NatRepemployees, family = "quasibinomial")
summary(mod)
```
...
mod <- lm(income_annual_all ~ Age + Gender * Ethnicity_collapsed*outsourcing_status + BORNUK_labelled + Region, income_data, weights = NatRepemployees)
summary(mod)
```
