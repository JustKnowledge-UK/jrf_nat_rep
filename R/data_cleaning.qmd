...
data <- data %>%
  mutate(
    income_group = case_when(income_weekly_all <= Region_two_thirds  ~ "Low",
                                 income_weekly_all <= Region_four_thirds ~ "Mid",
                                 income_weekly_all > Region_four_thirds ~ "High",
                                 TRUE ~ NA),
    income_group = factor(income_group, levels = c("Low", "Mid","High"), exclude=NA)
  ) %>%
  ungroup()
...
data <- data %>%
  mutate(
    Ethnicity_collapsed = case_when(
      ...
      Ethnicity %in% c(20) ~ "Don't think of myself as any of these",
      #prefer not to say
      Ethnicity %in% c(21) ~ "Prefer not to say",
...
data <- data %>%
  ...
  # ) %>%
  mutate(
    Gender = factor(Gender, levels = c("Male","Female","Other","Prefer not to say"))
  )
```
