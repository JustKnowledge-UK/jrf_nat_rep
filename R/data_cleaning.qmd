...
data <- data %>%
  ...
  mutate(
    ...
    income_annual_closed = case_when(INCOME_FREQ == 1 ~ INCOME_CLOSED_ANNUAL_labelled_midpoint,
                              ...
                              INCOME_FREQ == 3 ~ INCOME_CLOSED_WEEKLY_labelled_midpoint*52,
                              INCOME_FREQ == 4 ~ INCOME_CLOSED_HOURLY_labelled_midpoint*HOURS*52,
                              TRUE ~ NA),
...
# calculate the mean and sd, and 3x sd
mean = mean(x$income_annual_all, na.rm=T)
std = sd(x$income_annual_all, na.rm = T)
Tmin = mean-(3*std)
Tmax = mean+(3*std)
...
mean = mean(test$income_annual_all, na.rm = T)
std = sd(test$income_annual_all, na.rm = T)
Tmin = mean - (3 * std)
Tmax = mean + (3 * std)
